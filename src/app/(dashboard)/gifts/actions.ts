...
export async function saveGift(id: string | null, input: GiftInput) {
  ...
  const values = { ...parsed.data, description: parsed.data.description || null, emoji: parsed.data.emoji || null, item_key: parsed.data.item_key?.trim() || null, stock: parsed.data.stock === '' || parsed.data.stock == null ? null : parsed.data.stock };
  return staffAction({
