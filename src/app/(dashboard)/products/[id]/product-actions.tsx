...
export function ProductActions({ id, status }: { id: string; status: string }) {
  ...
  const change = async (s: 'draft' | 'published' | 'archived') => {
    const r = await setProductStatus(id, s);
  ...
  return (
    <>
      ...
      <ConfirmButton variant="destructive" title="Διαγραφή προϊόντος;" description="Δεν αναιρείται."
        onConfirm={async () => { const r = await deleteProduct(id); if (r.ok) router.replace('/products'); return r; }}>Διαγραφή</ConfirmButton>
    </>
  );
}
