-- ============================================================
-- 0019 AWARD KID FALLBACK: the game claims wins without picking a
-- kid, so awards had kid_id = null and the sponsor portal showed no
-- member ID / first name — the redemption handshake was empty.
-- When p_kid is null, attach the parent's first APPROVED kid that
-- meets the campaign's age requirements. Full re-create of
-- win_campaign_gift (0016) + a one-time backfill of existing awards.
-- ============================================================
...
begin
  ...
  -- no kid picked (the game plays on the family account): attach the first
  -- approved kid that fits the campaign's age window, so the sponsor gets
  -- a member ID + first name to hand the gift over to
  if p_kid is null then
    select k2.id into p_kid from kids k2
      where k2.parent_id = auth.uid() and k2.status = 'approved'
        and (not c.requirements ? 'min_age' or date_part('year', age(k2.dob))::int >= (c.requirements->>'min_age')::int)
        and (not c.requirements ? 'max_age' or date_part('year', age(k2.dob))::int <= (c.requirements->>'max_age')::int)
...
-- one-time backfill: attach the parent's first approved kid to kid-less awards
update awards a
  set kid_id = (
    select k.id from kids k
      where k.parent_id = a.user_id and k.status = 'approved'
      order by k.created_at limit 1)
  where a.kid_id is null;
